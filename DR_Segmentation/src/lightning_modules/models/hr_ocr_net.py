...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, ocr_config, num_classes=3, **kwargs):
        ...
        ocr_mid_channels = ocr_config.MID_CHANNELS
        ocr_key_channels = ocr_config.KEY_CHANNELS
    ...
    def forward(self, x):
        origin_h, origin_w = x.size(2), x.size(3)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        ...
        out_aux = F.interpolate(out_aux, size=(origin_h, origin_w),
                                mode='bilinear', align_corners=ALIGN_CORNERS)
        out = F.interpolate(out, size=(origin_h, origin_w),
                            mode='bilinear', align_corners=ALIGN_CORNERS)
...
def get_hr_ocr_model(cfg=MODEL_CONFIGS['hrnet18'], ocr_cfg=MODEL_CONFIGS['ocr'], **kwargs):
    model = HighResolutionNet(cfg, ocr_cfg, **kwargs)
    #model.init_weights("")
    return model

if __name__ == "__main__":
    import numpy as np
    image = np.ones([1, 3, 1024, 1024])
    image = torch.FloatTensor(image)
    model = get_hr_ocr_model(MODEL_CONFIGS['hrnet32'], MODEL_CONFIGS['ocr'])
    pred = model(image)
    print(len(pred))
    print(pred[0].size())
    print(pred[1].size())
