...
class EfficientNetV2(nn.Module):
    ...
    """
    def __init__(self, layer_infos, out_channels=1280, nclass=0, dropout=0.5, stochastic_depth=0.0,
...
def get_efficientnet_v2(model_name, pretrained, num_classes=0, dropout=0.15, stochastic_depth=0.15, **kwargs):
    residual_config = [MBConvConfig(*layer_config) for layer_config in get_efficientnet_v2_structure(model_name)]
    model = EfficientNetV2(residual_config, 1280, num_classes, dropout=dropout, stochastic_depth=stochastic_depth, block=MBConv, act_layer=nn.SiLU)
