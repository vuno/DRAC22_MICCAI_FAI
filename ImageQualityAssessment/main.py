...
def generate_ps_label(conf, ratio, data_dir):
    ratio = ratio / 100.
    ...
    data = {"image name": [], "image quality level": [], "split": []}
    ...
    if target_0_num > 0:
        ...
        data["split"] += split
        data["image quality level"] += label
    if target_1_num > 0:
...
def run(conf, u_df):
    ...
    dm = DRACDM("quality", conf.data_dir, conf.input_size, conf.batch_size, conf.num_workers, conf.balanced_sampling, conf.fold_idx, conf.auto_aug, u_df)
    lm = DRACLM("quality", conf.backbone, conf.lr, conf.optimizer, conf.loss, conf.beta1, conf.decay_step, conf.mix_up, num_classes)
...
def parse_argument():
    ...
    parser.add_argument('--save_dir', type=str, default='./outputs_drac/task2')
...
if __name__ == '__main__':
    ...
    for sl_idx in range(5):
        ...
        if fold_best_score < score.item():
            ...
            ratio += 20
            # generate ps label
            u_df = generate_ps_label(conf, ratio, conf.data_dir)
    print('*********************************************')
